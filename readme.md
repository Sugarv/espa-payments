...
1. Εγκατάσταση απαιτούμενων βιβλιοθηκών με την εντολή `composer update` σε υπολογιστή (ή το server) με εγκατεστημένο το [composer](https://getcomposer.org/)
- Ανέβασμα σε server (αν δεν έχει γίνει ήδη στο 1.)
- Ανέβασμα αρχείων csv στο φάκελο 'csv'.
...
2. Scans a folder for csv files (from [Bglossa](http://bglossa.ypepth.gr))
3. Parses each file, finding the requested user data
